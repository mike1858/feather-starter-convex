...
interface ContactsDetailPageProps {
...
export function ContactsDetailPage({
  item,
  isLoading,
  onSave,
  onDelete,
}: ContactsDetailPageProps) {
    ...
    <div className={`flex gap-8 max-w-4xl`}>
      ...
      <div className="flex-1 flex flex-col gap-6 p-6">
        ...
        <div className="flex items-center gap-4 text-xs text-muted-foreground pt-4 border-t border-border">
          {item._creationTime != null && (
            <span>Created {new Date(Number(item._creationTime)).toLocaleDateString()}</span>
          )}
          {item.updatedAt != null && (
            <span>Updated {new Date(Number(item.updatedAt)).toLocaleDateString()}</span>
          )}
        </div>
      </div>
