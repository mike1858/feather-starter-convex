...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const subtasks = await ctx.db.get(args.subtasksId);
    if (!subtasks) throw new Error(ERRORS.subtasks.NOT_FOUND);

    const patch: Record<string, unknown> = {};
    if (args.title !== undefined) patch.title = args.title;

    await ctx.db.patch(args.subtasksId, patch);
...
export const remove = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.delete(args.subtasksId);
...
export const reorder = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(args.subtasksId, { position: args.newPosition });
